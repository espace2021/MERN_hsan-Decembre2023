...
const Affichearticletable = () => {
    ...
    const handleDelete = (id, ref) => {
        if (window.confirm("supprimer Article O/N")) {
           ...
           .unwrap()
           .then(() => {
             if(error)   toast(`Article non supprimé  ${error} `, {
                position: "top-right",
                autoClose: 5000,
             ...
             else      toast(`Article ${ref} Supprimé`, {
                    position: "top-right",
                    autoClose: 5000,
                    hideProgressBar: false,
                    closeOnClick: true,
           ...
           .catch((err)=> toast(`Article non supprimé  ${err} `, {
            position: "top-right",
            autoClose: 5000,
            hideProgressBar: false,
            closeOnClick: true,
    ...
    if (error) return <p>Impossible d'afficher la liste des articles...{error}</p>
    return (
        <> 
            <div> 
